    ...
    <v-layout>
        <v-flex>
            <v-card>
                                    ...
                                    <v-layout wrap>
                                        <v-flex xs12 sm12 md12>
                                            <div v-if=" this.Indice ==-1">
                                                <v-text-field type="text" v-model="Item_editado.marca" label="Marca" counter maxlength="6"></v-text-field>

                                            </div>
                                            <div v-else>
                                                <v-text-field disabled="" type="text" v-model="Item_editado.marca" label="Mara" counter maxlength="6"></v-text-field>
                ...
                <!-- Tabla de datos -->
                <v-data-table :loading="Cargando" :headers="Encabezados" :items="Marcas" :search="Busqueda" class="elevation-1" rows-per-page-text="Numero de filas" no-results-text="No se ha encontrado resultados">
                    <!-- Barra de progreso -->
...
export default {
    ...
    methods: {
        ...
        Eliminar(item) {
            axios.delete(this.API_URL + item.marca).then((response) => {
        ...
        Aceptar() {
            ...
            if (this.Indice > -1) {
                //Editar
                axios.put(this.API_URL + this.Item_editado.marca, this.Item_editado).then((response) => {
                    this.Cerrar();
                    this.Limpiar();
                    this.Listar();
                }).catch((error) => {
        ...
        Validar() {
            ...
            this.Mensajes = [];
            if (this.Item_editado.marca.length < 3 || this.Item_editado.marca.length > 6) {
                this.Mensajes.push(">La marca debe tener entre 3 y 6 caracteres");
            ...
            if (this.Indice == -1) {
                if (this.Marcas.some(c => c.marca.toUpperCase() === this.Item_editado.marca.toUpperCase())) {
