                                    ...
                                    <v-layout wrap>
                                        <v-flex xs12 sm12 md12>
                                            <div v-if=" this.Indice ==-1">
                                                <v-text-field type="text" v-model="Item_editado.linea" label="Linea" counter maxlength="6"></v-text-field>

                                            </div>
                                            <div v-else>
                                                <v-text-field disabled="" type="text" v-model="Item_editado.linea" label="Linea" counter maxlength="6"></v-text-field>
...
<v-dialog v-model="Dialogo2" max-width="500px">
                        <v-card>
                            ...
                            <v-card-text>Estás a punto de eliminar la linea: {{ Item_editado.linea }}
                            </v-card-text>
                            <v-card-actions>
                                <v-spacer></v-spacer>
                ...
                <!-- Tabla de datos -->
                <v-data-table :loading="Cargando" :headers="Encabezados" :items="Lineas" :search="Busqueda" class="elevation-1" rows-per-page-text="Numero de filas" no-results-text="No se ha encontrado resultados">
                    <!-- Barra de progreso -->
...
export default {
    ...
    methods: {
        ...
        Aceptar2() {
            axios.delete(this.API_URL + this.Item_editado.linea).then((response) => {
        ...
        Aceptar() {
            ...
            if (this.Indice > -1) {
                //Editar
                axios.put(this.API_URL + this.Item_editado.linea, this.Item_editado).then((response) => {
                    this.Cerrar();
                    this.Limpiar();
                    this.Listar();
                }).catch((error) => {
        ...
        Validar() {
            ...
            this.Mensajes = [];
            if (this.Item_editado.linea.length < 3 || this.Item_editado.linea.length > 6) {
                this.Mensajes.push(">La linea debe tener entre 3 y 6 caracteres");
            ...
            if (this.Indice == -1) {
                if (this.Lineas.some(c => c.linea.toUpperCase() === this.Item_editado.linea.toUpperCase())) {
