    ...
    <v-layout>
        <v-flex>
            <v-card>
                <v-toolbar flat>
                    ...
                    <v-dialog v-model="Dialogo2" max-width="500px">
                        <v-card>
                            ...
                            <v-card-text>Estás a punto de eliminar el kit: {{ Item_editado.articulo }}
                            </v-card-text>
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                ...
                                <v-container grid-list-md>
                                    <v-layout wrap>
                                        <v-flex xs12 sm12 md12>
                                            <v-text-field type="number" v-if="this.Indice ===-1" v-model="Item_editado.articulo" label="Nombre" :min="1" prefix="KIT"></v-text-field>
                                            <v-text-field type="text" v-else disabled v-model="Item_editado.articulo" label="Nombre" :min="1"></v-text-field>
                                        ...
                                        <v-flex xs6>
                                            <v-text-field type="number" disabled :min="0" v-model="Item_editado.precio1" label="Precio"></v-text-field>
                                        </v-flex>
                                        <v-flex xs6>
                                        ...
                                        <v-layout align-center justify-center row fill-height>
                                            <v-flex xs6>
                                                <v-autocomplete v-model="Componente" :items="Productos_Lista" item-text="descrip" item-value="articulo" label="Producto" prepend-icon="search">
                                                </v-autocomplete>
                                            </v-flex>
                                            <v-flex xs2>
                                        ...
                                        <v-flex xs12>
                                            <v-data-table :headers="Encabezados2" :items="Componentes_Lista" rows-per-page-text="Numero de filas" no-results-text="No hay componentes en el kit">
                                                <!-- lista de datos -->
                                                <template v-slot:items="props">
                                                    ...
                                                    <td>
                                                        <!-- Boton eliminar -->
                                                        <v-icon small @click="EliminarComp(props.item)">
                                                            delete
                                                        </v-icon>
                ...
                <v-data-table item-key="articulo" :loading="Cargando" :headers="Encabezados" :items="Kits" :search="Busqueda" class="elevation-1" rows-per-page-text="Numero de filas" no-results-text="No se ha encontrado resultados">
                    <!-- Barra de progreso -->
                    ...
                    <template v-slot:expand="props">
                        <v-card flat>
                            <v-card-text>
                                <div v-for="(comp, index) in Componentes_Expand" :key="index">
...
export default {
    ...
    async created() {
        this.Listar();
        await axios.get(this.API_URL + "listarnombre").then((response) => {
    ...
    methods: {
        async Editar(item) {
            await axios.get("compparts/lista/" + item.articulo).then((response) => {
        ...
        async Listar() {
            this.Cargando = true;
            this.Kits = [];
            await axios.get(this.API_URL + "listark").then((response) => {
        ...
        async Expandir(props) {
            await axios.get("compparts/componentes/" + props.item.articulo).then((response) => {
        ...
        AgregarComp(com) {
            ...
            if (!this.Componentes_Lista.some(c => c.componente.toUpperCase() === prod.articulo.toUpperCase())) {
                if (this.Indice == -1) {
        ...
        async Aceptar2() {
            await axios.delete('compparts/delkit/' + this.Item_editado.articulo).then((response) => {}).catch((error) => {});
            await axios.delete('componens/' + this.Item_editado.articulo).then((response) => {

            }).catch((error) => {});
            await axios.delete(this.API_URL + this.Item_editado.articulo).then((response) => {
        ...
        async Aceptar() {
            ...
            if (this.Indice > -1) {
                //Editar
                await axios.delete("compparts/DelKit/" + this.Item_editado.articulo).then((response) => {

                }).catch((error) => {
                    console.log(error);
                });
                await axios.post("compparts/AddKit/", this.Componentes_Lista).then((response) => {

                }).catch((error) => {
                    console.log(error);
                });
                await axios.put(this.API_URL + this.Item_editado.articulo, this.Item_editado).then((response) => {
                ...
                await axios.post(this.API_URL, this.Item_editado).then((response) => {

                }).catch((error) => {
                    console.log(error);
                });
                await axios.post("componens/", {
                    "articulo": this.Item_editado.articulo,
                    "coment": null,
                    "minimo": null,
        ...
        Validar() {
            ...
            if (this.Item_editado.articulo.length < 3 || this.Item_editado.articulo.length > 15) {
            ...
            if (this.Indice == -1) {
                if (this.Kits.some(c => c.articulo.toUpperCase() === this.Item_editado.articulo.toUpperCase())) {
