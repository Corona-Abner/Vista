                ...
                <v-toolbar flat>
                    ...
                    <v-dialog v-model="Dialogo2" max-width="500px">
                        <v-card>
                            ...
                            <v-card-text>Estás a punto de eliminar el producto: {{ Item_editado.articulo }}
                            </v-card-text>
                            <v-card-actions>
                                <v-spacer></v-spacer>
...
export default {
    ...
    created() {
        axios.get('lineas').then((response) => {
    ...
    methods: {
        ...
        Aceptar2() {
            axios.delete(this.API_URL + this.Item_editado.articulo).then((response) => {
        ...
        Aceptar() {
            ...
            if (this.Indice > -1) {
                //Editar
                axios.put(this.API_URL + this.Item_editado.articulo, this.Item_editado).then((response) => {
                    this.Cerrar();
                    this.Limpiar();
                    this.Listar();
                }).catch((error) => {
        ...
        Validar() {
            ...
            if (this.Item_editado.claveunidad.length < 3 || this.Item_editado.claveunidad.length > 10) {
                this.Mensajes.push(">La clave de unidad debe tener entre 3 y 10 caracteres");
                this.Validacion = true;
            }
            //Verifica si el nombre esta repetido
            if (this.Indice == -1) {
                if (this.Productos.some(c => c.articulo.toUpperCase() === this.Item_editado.articulo.toUpperCase())) {
